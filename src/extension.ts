...
const channel = vscode.window.createOutputChannel("direnv vscode profile", { log: true });
...
export const activate = async (context: ExtensionContext) => {
    ...
    const evaluateProfileChange = async () => {
        ...
        const currentProfile = vscode.workspace.getConfiguration("workbench").get("settings.application.profile") || "default";
        ...
        try {
            await vscode.commands.executeCommand("workbench.profiles.actions.switchProfile", targetProfile)
            channel.info(`Switched VSCode profile to: ${targetProfile}`)
            vscode.window.showInformationMessage(`Switched VSCode profile to: ${targetProfile}`)
        } catch (err) {
    ...
    const configWatcher = vscode.workspace.onDidChangeConfiguration(async (event) => {
        if (event.affectsConfiguration("terminal.integrated.env") || event.affectsConfiguration("direnv")) {
            await evaluateProfileChange()
        }
    })
